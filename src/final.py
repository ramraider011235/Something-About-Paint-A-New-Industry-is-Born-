...
census['epa_est_leftover'] = census['epa_est_sold_gal'] * 0.10
census['epa_est_collectable'] = census['epa_est_leftover'] * 0.70
census['company_cur_mkt_sh'] = census['epa_est_collectable'] * 0.4

x = np.arange(len(census['epa_est_sold_gal']))
step = int((int(census['epa_est_sold_gal'].max()) - int(census['epa_est_collectable'].min())) / 20)
y = np.arange(0, int(census['epa_est_sold_gal'].max()) + step, step)
                     
fig = plt.figure(figsize=(20, 10))

ax = fig.add_subplot(111, ylabel='Dollars In Billions $') 
census[['epa_est_sold_gal', 'epa_est_leftover', 'epa_est_collectable', 'company_cur_mkt_sh']].plot(kind = 'bar', ax=ax, width = 2)
...
paintcare_co[[
    '2015', '2016', '2017', '2018', '2019']].plot(kind = 'bar',ax=ax, width = .75, align = 'edge')
...
y1 = np.arange(int(paint_collected['total_collected'].min()) - 500, int(paint_collected['total_collected'].max()) + 500, 1000)
...
paint_processed[[
    'paint_processed_gal', 'paint_packaged_gal','paint_shipped_gal']].plot(kind = 'bar',width = .5, align = 'edge', ax = ax1)
...
profit_loss[[
    '2017', '2018', '2019']].plot(kind='bar', ax = ax, width = .85, align = 'edge')
...
proforma[[
    'state_recycle', 'company_collection', 'company_produced']].plot(kind = 'bar', ax = ax, width = .85, align = 'edge')
...
ax = fig.add_subplot(111, ylabel = 'Volume (gal)',xlabel = 'Weekly Report') 
weekly_kpi[[
    'recyclingincome','productincome']].plot(kind = 'bar', ax = ax, width = .85, align = 'center')
...
df = df.drop([
    0,1,2,3,4,8,15, 35, 38, 42, 36,17,18,22,23,27,28,29,31,32,33,34,37,39,6,14,16,20,21,24,26,41,43,47,48,7,12,13,25,30,44,46
])
...
df[[
    '2015','2016', '2017','2018'
]].plot(kind = 'bar', ax = ax, width = .5, align = 'center')
